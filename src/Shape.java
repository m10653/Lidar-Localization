...
public class Shape {
	...
	public ArrayList<Point> surroundingPoints(Point point){
		...
		for(int x=0;x<vertices.size();x++){
			if(vertices.get(x).getY()>input.getY()&&
				...
				surroundingPoints.add(vertices.get(x));
				surroundingPoints.add(vertices.get((x+1)%vertices.size()));
			}	
		}
	...
	public int rightIntersects(Point point){
		ArrayList<Point> surroundingPoint= surroundingPoints(point);
		int count=0;
		double y= point.getY();
		double x = point.getX();
		for(int b=0;b<surroundingPoint.size();b+=2){
			if(x<=intersectingX(surroundingPoint.get(b),surroundingPoint.get(b+1),point)){
				count++;
			}
			if(surroundingPoint.get(b).getX()==surroundingPoint.get(b+1).getX()
					&&x>(surroundingPoint.get(b).getX())){
				count++;
			}
		}
